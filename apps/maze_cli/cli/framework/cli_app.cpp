...
auto IsConfigToken(const std::string& token) -> bool {
  return token == "-c" || token == "--config";
}
...
auto ConsumeOptionValue(const std::string& token,
  ...
  if (IsConfigToken(token)) {
    ...
    ++index;  // startup already consumed the config path
    return outcome;
  }
...
void CliApp::print_help(std::ostream& out) const {
  ...
  out << "  -c, --config <file>  Set config file path\n";
