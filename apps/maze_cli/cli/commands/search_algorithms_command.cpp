...
auto DimensionLabel(MazeSolver::SolverAlgorithmType /*type*/) -> const char* {
  return "2D";
}
...
void PrintSupportedSearchAlgorithms(std::ostream& out) {
  ...
  const std::vector<MazeSolver::SolverAlgorithmType> algorithms = {
      MazeSolver::SolverAlgorithmType::BFS,
      MazeSolver::SolverAlgorithmType::DFS,
      MazeSolver::SolverAlgorithmType::ASTAR,
      MazeSolver::SolverAlgorithmType::DIJKSTRA,
      MazeSolver::SolverAlgorithmType::GREEDY_BEST_FIRST};
  for (const auto algorithm : algorithms) {
    out << "  - " << MazeSolver::AlgorithmName(algorithm) << " ["
        << DimensionLabel(algorithm) << "]\n";
  }
}
