...
void RunSolverAndRender(MazeDomain::MazeGrid& maze_grid,
                        ...
                        std::ostream& err) {
  out << "--- " << solver_label << " Solving & Image Generation ("
  ...
  if (!kRenderResult.ok) {
    err << kDisplayName << ": " << kRenderResult.error << std::endl;
  } else {
    out << "Rendered " << kRenderResult.frames_written << " frames for "
        << kDisplayName << " (maze generated by " << algo_info.name << ") in "
        << kRenderResult.output_folder << std::endl;
  }
  ...
  out << kGreenColor << std::fixed << std::setprecision(3) << "Time for "
      << solver_label << " solving & image generation: " << kTimeTaken.count()
      << " s" << kResetColor << std::endl;
}
...
void RunGenerationForAlgorithm(const Config::AppConfig& config,
  ...
  for (const auto& solver_info : config.maze.search_algorithms) {
    RunSolverAndRender(maze_grid, topology, algo_info, config, solver_info.type,
