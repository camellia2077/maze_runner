...
auto RenderFrameToBufferImpl(const SearchFrame& frame,
                             const MazeDomain::MazeGrid& maze_ref,
  ...
  out_buffer.width = kImageSize.final_img_width;
  out_buffer.height = kImageSize.final_img_height;
...
auto RenderFrameToBuffer(const SearchFrame& frame,
                         const MazeDomain::MazeGrid& maze_ref,
                         const Config::AppConfig& config,
                         RenderBuffer& out_buffer, std::string& error) -> bool {
  return RenderFrameToBufferImpl(frame, maze_ref, config, out_buffer, error);
}
...
auto WritePngFile(const std::string& path, const RenderBuffer& buffer,
  ...
  const int kWriteOk = stbi_write_png(path.c_str(), buffer.width, buffer.height,
                                      buffer.channels, buffer.pixels.data(),
                                      buffer.width * buffer.channels);
  if (kWriteOk == 0) {
...
auto RenderSearchResult(const SearchResult& result,
  ...
  for (size_t frame_index = 0; frame_index < result.frames_.size();
    ...
    if (!RenderFrameToBuffer(result.frames_[frame_index], maze_ref, config,
                             buffer, save_error)) {
      render_result.ok = false;
      render_result.error = save_error;
      return render_result;
    }
    const fs::path kFramePath =
        BuildFramePath(folder_path, static_cast<int>(frame_index));
    if (!WritePngFile(kFramePath.string(), buffer, save_error)) {
      render_result.ok = false;
      render_result.error = save_error;
      return render_result;
    }
