...
from . import task_splitter

# --- 配置 ---
# 定义构建目录和可执行文件的名称
BUILD_DIR_RELEASE = "build"
BUILD_DIR_DEBUG = "build_debug"
EXECUTABLE_NAME = "maze_cli.exe"
...
def run_clang_tidy_per_file(build_path, tidy_mode):
    ...
    def run_single(source_path, index_value):
        print(f"[{index_value}/{total}] clang-tidy: {source_path}", flush=True)
        tidy_command = base_command + [source_path]
        ...
        output_lines = result.stdout.splitlines(keepends=True) if result.stdout else []
        task_header = f"[{index_value}/{total}] [1/CHECK] Analyzing: {source_path}\n"
        task_lines = [task_header] + output_lines
        return {
            "index": index_value,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for index, source in enumerate(sources, start=1):
            futures.append(executor.submit(run_single, source, index))
    ...
    full_log_lines = []
    for item in results:
        full_log_lines.extend(item["task_lines"])

    tasks_dir = Path(build_path) / "tasks"
    created_count = task_splitter.split_tidy_logs(full_log_lines, tasks_dir)
    print(f"--- Created {created_count} task log files in {tasks_dir}")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["check", "fix"],
        default="fix",
        help="clang-tidy mode when target=tidy",
    )
    parser.add_argument(
        "--build-type",
    ...
    parser.add_argument(
        ...
        choices=["O0", "O1", "O2", "O3"],
        default=None,
        help="Override release optimization level",
    )
    parser.add_argument(
        "--release-extra-flags",
        default=None,
        help="Override extra release compiler flags",
    )
    ...
    strip_group.add_argument(
        ...
        dest="strip_binaries",
        action="store_true",
        help="Enable binary stripping for release builds",
    )
    strip_group.add_argument(
        "--no-strip",
        dest="strip_binaries",
        action="store_false",
        help="Disable binary stripping for release builds",
    )
    parser.set_defaults(strip_binaries=None)
    parser.add_argument(
        "--static-link",
        action="store_true",
        help="Enable static linking for release executable",
    )
...
def main():
    ...
    if args.release_opt_level:
        cmake_command.append(f"-DMAZE_RELEASE_OPT_LEVEL={args.release_opt_level}")
    if args.release_extra_flags is not None:
        cmake_command.append(f"-DMAZE_RELEASE_EXTRA_FLAGS={args.release_extra_flags}")
    if args.strip_binaries is not None:
        cmake_command.append(
            f"-DMAZE_RELEASE_STRIP_BINARIES={'ON' if args.strip_binaries else 'OFF'}"
        )
    if args.static_link:
        cmake_command.append("-DMAZE_FORCE_STATIC_LINK=ON")
    if args.generator:
    ...
    if args.target == "tidy":
        run_clang_tidy_per_file(build_path, args.tidy_mode)
    else:
